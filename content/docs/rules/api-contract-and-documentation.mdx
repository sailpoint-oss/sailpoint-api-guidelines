...
## OpenAPI Authoring Context

High-quality OpenAPI is the fastest path to consistent APIs: it enables design review,
documentation, SDK generation, and automated checks.
...
- Every operation should have: a clear `summary`, a descriptive `description` (include edge cases and compatibility notes), at least one success example and one error example.
- Use `externalDocs` to link to supporting guidance when a concept is too large to fit inline.
