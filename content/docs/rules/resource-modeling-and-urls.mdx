...
These rules cover how to model domain concepts as resources and expose them via stable, consistent
URLs.
...
## Resource & URL Design
<RuleHeader id="500" level="MUST" title="Avoid /api base path" />
### #500 - Avoid /api Base Path

**Intent**: keep public URLs clean and avoid redundant, implementation-driven path segments.

Avoid redundant base path segments like `/api` in public-facing URL paths.
...
<RuleHeader id="501" level="MUST" title="Define canonical URL forms" />
...
<RuleHeader id="502" level="SHOULD" title="Model resources (avoid action endpoints)" />
### #502 - Model Resources (Avoid Action Endpoints)

**Intent**: keep APIs consistent, predictable, and aligned with HTTP semantics.
...
<RuleHeader id="503" level="SHOULD" title="Model business processes" />
### #503 - Model Business Processes

**Intent**: enable real customer workflows without forcing clients to stitch together fragile sequences.
...
<RuleHeader id="504" level="SHOULD" title="Define useful resources" />
### #504 - Define Useful Resources

**Intent**: keep APIs practical and efficient for the most common use cases.
...
<RuleHeader id="505" level="SHOULD" title="Keep URLs verb-free" />
### #505 - Keep URLs Verb-free

**Intent**: keep the URL space stable and expressive via HTTP methods.
...
<RuleHeader id="506" level="MUST" title="Use domain-specific resource names" />
### #506 - Use Domain-specific Resource Names

**Intent**: make the API intuitive for customers/partners and stable across internal refactors.
...
<RuleHeader id="507" level="MUST" title="Nest sub-resources only when tightly coupled" />
### #507 - Nest Sub-resources Only When Tightly Coupled

**Intent**: express ownership/scoping clearly and prevent ambiguous identifiers.
...
<RuleHeader id="508" level="SHOULD" title="Prefer non-nested URLs for independent resources" />
### #508 - Prefer Non-nested URLs for Independent Resources

**Intent**: keep URLs stable, shallow, and reusable when resources have independent identity/lifecycle.
...
<RuleHeader id="509" level="SHOULD" title="Limit resource types" />
### #509 - Limit Resource Types

**Intent**: keep APIs learnable and prevent uncontrolled surface area growth.
...
<RuleHeader id="510" level="SHOULD" title="Limit nesting depth" />
...
<RuleHeader id="512" level="MUST" title="Use URL-friendly IDs" />
### #512 - Use URL-friendly IDs

**Intent**: avoid encoding ambiguity and ensure IDs work across tooling, proxies, and clients.
...
<RuleHeader id="513" level="MUST" title="Never include customer org names in paths" />
...
<RuleHeader id="514" level="MUST" title="Avoid sequential numeric IDs" />
### #514 - Avoid Sequential Numeric IDs

**Intent**: reduce enumeration risk and avoid leaking internal scale/ordering.
...
## Hypermedia & Links
<RuleHeader id="511" level="MUST" title="Target REST maturity level 2" />
### #511 - Target REST Maturity Level 2
