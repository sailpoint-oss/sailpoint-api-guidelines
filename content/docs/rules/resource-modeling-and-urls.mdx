...
When an operation doesn't map to a standard CRUD verb, create a noun-like command or request resource:
...
Avoid exposing many near-duplicate resources or designs that require N additional calls for the common case.
...
- MUST use resource URLs (nouns), not action/RPC endpoints.
- MUST use HTTP methods correctly (see [#401]) and respect method properties (see [#402]).
- MUST use standard, semantically correct status codes (see [#403]).
- MUST use standard content negotiation and media types (see [#705]).
- MUST use a standardized error contract (Problem Details) (see [#404]).
