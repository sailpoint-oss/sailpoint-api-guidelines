...
These rules define request/query conventions (filtering, sorting, pagination) so list/search endpoints
are consistent across APIs.
...
<RuleHeader id="600" level="MUST" title="Use conventional query parameters" />
...
<RuleHeader id="601" level="MUST" title="Define collection formats for header/query params" />
### #601 - Define Collection Formats for Header/query Params

**Intent**: ensure clients can reliably generate requests and servers can parse them.
...
<RuleHeader id="603" level="MUST" title="Document implicit filters and default sorting" />
...
<RuleHeader id="604" level="MUST" title="Use one query format across the API" />
### #604 - Use One Query Format Across the API

**Intent**: make querying consistent so documentation, parsing, and shared client/server logic can be reused.
...
## Pagination
<RuleHeader id="602" level="MUST" title="Support offset pagination for list endpoints" />
